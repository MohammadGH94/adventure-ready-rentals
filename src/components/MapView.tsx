      ...
      listings.forEach((listing) => {
        if (listing.location_lat && listing.location_lng && map.current) {
          ...
          const popup = new mapboxgl.Popup({ 
            ...
            <div class="p-0 w-72">
              <div class="relative">
                <div id="carousel-${listing.id}" class="relative w-full h-44 bg-gray-100 overflow-hidden">
                  ...
                  ${photos.length > 1 ? `
                    <button onclick="window.navigateCarousel('${listing.id}', -1)" 
                            class="absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-1.5 shadow-lg transition-all">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                    </button>
                    <button onclick="window.navigateCarousel('${listing.id}', 1)" 
                            class="absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-1.5 shadow-lg transition-all">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                    </button>
                    <div class="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black/70 text-white text-xs px-2 py-0.5 rounded">
                      1 / ${photos.length}
                    </div>
                  ` : ''}
                </div>
              ...
              <div class="p-3">
                <div class="flex items-start justify-between mb-2">
                  <h3 class="font-semibold text-base text-gray-900 flex-1">${listing.title}</h3>
                  <span class="text-base font-bold text-emerald-600 ml-2">$${listing.price_per_day}/day</span>
                </div>
                <p class="text-xs text-gray-600 mb-2 line-clamp-2">${truncatedDesc}</p>
                ...
                <p class="text-xs text-gray-500 mb-2.5">📍 Approximate location (for privacy)</p>
                <button onclick="window.dispatchEvent(new CustomEvent('mapListingClick', {detail: '${listing.id}'}))" 
                        class="w-full px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-colors">
                  View Full Details
                </button>
              </div>
            </div>
