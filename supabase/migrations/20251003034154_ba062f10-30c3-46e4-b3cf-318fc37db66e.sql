-- Drop and recreate get_listing_for_booking with owner information
DROP FUNCTION IF EXISTS public.get_listing_for_booking(uuid);

CREATE FUNCTION public.get_listing_for_booking(listing_id uuid)
RETURNS TABLE(
  id uuid,
  title text,
  description text,
  categories text[],
  price_per_day numeric,
  location_lat numeric,
  location_lng numeric,
  photos text[],
  min_rental_days integer,
  max_rental_days integer,
  delivery_available boolean,
  delivery_radius numeric,
  delivery_fee numeric,
  pickup_instructions text,
  rules_and_requirements text,
  deposit_amount numeric,
  insurance_required boolean,
  add_ons jsonb,
  bulk_pricing jsonb,
  inventory_count integer,
  rented_as_kit boolean,
  owner_id uuid,
  owner_first_name text,
  owner_profile_image_url text,
  owner_created_at timestamp with time zone
)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = 'public'
AS $$
  SELECT 
    l.id,
    l.title,
    l.description,
    l.categories,
    l.price_per_day,
    l.location_lat,
    l.location_lng,
    l.photos,
    l.min_rental_days,
    l.max_rental_days,
    l.delivery_available,
    l.delivery_radius,
    l.delivery_fee,
    l.pickup_instructions,
    l.rules_and_requirements,
    l.deposit_amount,
    l.insurance_required,
    l.add_ons,
    l.bulk_pricing,
    l.inventory_count,
    l.rented_as_kit,
    u.id as owner_id,
    u.first_name as owner_first_name,
    u.profile_image_url as owner_profile_image_url,
    u.created_at as owner_created_at
  FROM public.listings l
  INNER JOIN public.users u ON l.owner_id = u.id
  WHERE l.id = listing_id 
    AND l.is_available = true 
    AND l.listing_status = 'active';
$$;